<div id="highlights" class="">

    <div class="jumbotron jumbotron-fluid jumbotron-img">
        <div class="container">
            <div class="col">
            <div class="div-logo"> 
                <img class="logo-intersep" src="<?php echo get_stylesheet_directory_uri(); ?>/assets/img/logo_highlights.svg">
               
            </div>
            <div class="div-button">
                <a href="<?php echo get_permalink( get_page_by_path( 'sobre' ) ) ?>">
                    <button type="button" class="btn btn-outline-primary" >DISCOVER</button>
                </a>
                <a href="<?php echo get_permalink( get_page_by_path( 'contato' ) ) ?>">
                    <button role="button" type="button" class="btn btn-outline-secondary">CONTACT US</button>
                </a>
            </div>
                <!-- <h1 class="display-7 text">Núcleo de Estudos em Modelagem Conceitual e Ontologias</h1>
                <a class="btn btn-primary buttom-highlights" href="<?php echo get_permalink('sobre')?>" role="button">CONHEÇA O PROJETO</a> -->
                
            </div>
        </div>
    </div>

</div>